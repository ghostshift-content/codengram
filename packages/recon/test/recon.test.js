...
test('FAIL-CLOSED: with no Claude Lead, a scan publishes technical architecture only — never folder clusters as features', async () => {
  ...
  g.close()
  // the RENDERED bundle is honest: a blocked banner + an architecture view, never fake feature files
  const pub = path.join(snapshotDir(dataRoot, project.id, res.snapshotId), 'publications', res.pubId, 'phase1-maps')
  assert.ok(fs.readFileSync(path.join(pub, 'README.md'), 'utf8').includes('SEMANTIC PLANNING BLOCKED'))
  assert.ok(fs.existsSync(path.join(pub, 'consolidated', 'architecture.md')))
  assert.ok(fs.readFileSync(path.join(pub, 'consolidated', 'architecture.md'), 'utf8').includes('NOT business features'))
  assert.ok(!fs.existsSync(path.join(pub, 'features')), 'no features/ dir when blocked')
  const manifest = JSON.parse(fs.readFileSync(path.join(pub, 'manifest.json'), 'utf8'))
  assert.equal(manifest.executed_planner, 'blocked')
  assert.equal(manifest.semantic, false)
  assert.equal(manifest.feature_count, 0)
